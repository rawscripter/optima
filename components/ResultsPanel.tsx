...
export function ResultsPanel({ files, onDownloadZip }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.04] p-5 flex flex-col gap-4">
      ...
      <div className="rounded-lg overflow-hidden border border-base">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-base bg-overlay">
              <th className="text-left px-3 py-2 text-4 font-medium">File</th>
              <th className="text-right px-3 py-2 text-4 font-medium">Original</th>
              <th className="text-right px-3 py-2 text-4 font-medium">WebP</th>
              <th className="text-right px-3 py-2 text-4 font-medium">Saved</th>
            </tr>
          </thead>
          <tbody>
            {done.map((f) => {
              ...
              return (
                <tr key={f.id} className="border-b border-base last:border-0 hover:bg-overlay transition-colors">
                  <td className="px-3 py-2 text-2 truncate max-w-[200px]">
                    {f.resultFilename ?? f.file.name}
                  </td>
                  <td className="px-3 py-2 text-right text-3">{fmt(f.file.size)}</td>
                  <td className="px-3 py-2 text-right text-2">{fmt(out)}</td>
                  <td className={`px-3 py-2 text-right font-semibold ${pct > 50 ? 'text-emerald-400' : pct > 20 ? 'text-yellow-400' : 'text-3'}`}>
                    -{pct}%
                  </td>
                </tr>
              );
